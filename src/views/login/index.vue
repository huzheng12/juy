        ...
        if (valid) {
          ...
          try {
            ...
            if (res.success) {
              // console.log('登录成功');
              // console.log(res.obj);
              this.$store.dispatch("setRoleId", res.obj.role[0].id);
              this.$store.dispatch("setAgentId", res.obj.id);
              sessionStorage.setItem('routerInfo', JSON.stringify(res.obj.permission))
              this.$store.commit('UPDATE_TOKEN', { token: res.obj.token })
              sessionStorage.setItem('rolestatus', '代理方')
              this.$store.dispatch("setRoles", ["agent"]);
              res.obj.role.map(item => {
                if (item.id === 18) {
                  sessionStorage.setItem('rolestatus', '运营方');
                  this.$store.dispatch("setRoles", ["superAdmin"]);
                }
              })
